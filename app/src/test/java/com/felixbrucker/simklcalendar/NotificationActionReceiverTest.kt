...
class NotificationActionReceiverTest {
    ...
    fun setUp() {
        ...
        coEvery { calendarRepositoryMock.markEpisodeWatched(any(), any(), any(), any()) } returns Result.success(Unit)
        coEvery { calendarRepositoryMock.markSeasonWatched(any(), any(), any()) } returns Result.success(true)
        coEvery { calendarRepositoryMock.updateMediaStatus(any(), any()) } returns Unit
    ...
    fun testOnReceiveMarkItemWatched() {
        ...
        coVerify(timeout = 3000) { calendarRepositoryMock.markEpisodeWatched(100, 1, 1, MediaType.TV) }
    ...
    fun testOnReceiveMarkMovieWatched() {
        ...
        coEvery { calendarRepositoryMock.markMovieWatched(200) } returns Result.success(Unit)

        receiver.onReceive(context, intent)

        verify(timeout = 3000) { pendingResult.finish() }
        coVerify(timeout = 3000) { calendarRepositoryMock.markMovieWatched(200) }
        coVerify(timeout = 3000) { notificationManagerMock.dismissNotification(item) }
    }
    ...
    fun testOnReceiveMarkSeasonWatched() {
        ...
        verify(timeout = 3000) { pendingResult.finish() }
        coVerify(timeout = 3000) { calendarRepositoryMock.markSeasonWatched(100, 1, MediaType.TV) }
        coVerify(timeout = 3000) { notificationManagerMock.dismissNotification(item) }
    }
    ...
    fun testOnReceiveDownloadItem() {
        ...
        verify(timeout = 3000) { pendingResult.finish() }
        coVerify(timeout = 3000) { calendarRepositoryMock.updateMediaStatus("v2_100_1_1", MediaStatus.WANTED) }
