...
class CalendarViewModelTest {
    ...
    private lateinit var watchlistRepositoryMock: WatchlistRepository
    ...
    fun setUp() {
        ...
        watchlistRepositoryMock = mockk(relaxed = true)
        ...
        every { watchlistRepositoryMock.watchlistItems } returns watchlistItemsFlow
        ...
        every { calendarRepositoryMock.watchedEpisodes } returns flowOf(emptyList())
    ...
    fun testMarkEpisodeWatchedSuccessAndFailure() = runTest {
        val viewModel = createViewModel()
        coEvery { calendarRepositoryMock.markEpisodeWatched(1, 1, 2, MediaType.TV) } returns Result.success(Unit)
        ...
        var failureMsg = ""
        coEvery { calendarRepositoryMock.markEpisodeWatched(1, 1, 2, MediaType.TV) } returns Result.failure(Exception("Failed to mark episode as watched"))
        viewModel.markEpisodeWatched(
            simklId = 1,
            season = 1,
            episodeNumber = 2,
    ...
    fun testMarkSeasonWatchedSuccessAndFailure() = runTest {
        val viewModel = createViewModel()
        coEvery { calendarRepositoryMock.markSeasonWatched(1, 1, MediaType.TV) } returns Result.success(false)
        ...
        var failureCalled = true
        var failureMsg = ""
        coEvery { calendarRepositoryMock.markSeasonWatched(1, 1, MediaType.TV) } returns Result.failure(Exception("Failed to mark season as watched"))
        viewModel.markSeasonWatched(
            simklId = 1,
            season = 1,
            mediaType = MediaType.TV,
    ...
    fun testMarkMovieWatchedSuccessAndFailure() = runTest {
        val viewModel = createViewModel()
        coEvery { calendarRepositoryMock.markMovieWatched(1) } returns Result.success(Unit)
        ...
        var failureCalled = true
        var failureMsg = ""
        coEvery { calendarRepositoryMock.markMovieWatched(1) } returns Result.failure(Exception("Failed to mark movie as watched"))
        viewModel.markMovieWatched(
            simklId = 1,
            primaryKey = "v2_1_theater",
            showTitle = "Movie Title"
    ...
    fun testMarkEpisodeUnwatchedSuccessAndFailure() = runTest {
        val viewModel = createViewModel()
        coEvery { calendarRepositoryMock.markEpisodeUnwatched(1, 1, 2, MediaType.TV) } returns Result.success(Unit)
        ...
        var failureCalled = true
        var failureMsg = ""
        coEvery { calendarRepositoryMock.markEpisodeUnwatched(1, 1, 2, MediaType.TV) } returns Result.failure(Exception("Failed to mark episode as unwatched"))
        viewModel.markEpisodeUnwatched(
            simklId = 1,
            season = 1,
            episodeNumber = 2,
    ...
    fun testMarkSeasonUnwatchedSuccessAndFailure() = runTest {
        val viewModel = createViewModel()
        coEvery { calendarRepositoryMock.markSeasonUnwatched(1, 1, MediaType.TV) } returns Result.success(Unit)
        ...
        var failureCalled = true
        var failureMsg = ""
        coEvery { calendarRepositoryMock.markSeasonUnwatched(1, 1, MediaType.TV) } returns Result.failure(Exception("Failed to mark season as unwatched"))
        viewModel.markSeasonUnwatched(
            simklId = 1,
            season = 1,
            mediaType = MediaType.TV,
    ...
    fun testMarkMovieUnwatchedSuccessAndFailure() = runTest {
        val viewModel = createViewModel()
        coEvery { calendarRepositoryMock.markMovieUnwatched(1) } returns Result.success(Unit)
        ...
        var failureCalled = true
        var failureMsg = ""
        coEvery { calendarRepositoryMock.markMovieUnwatched(1) } returns Result.failure(Exception("Failed to mark movie as unwatched"))
        viewModel.markMovieUnwatched(
            simklId = 1,
            primaryKey = "v2_1_theater",
            showTitle = "Movie Title"
